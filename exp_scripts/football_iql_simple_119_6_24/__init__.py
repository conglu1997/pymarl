...
label = "gfootball_simple119_6_24"
config = "qmix"
    ...
    for rnn_dim in [128, 256]:
        for rewards in ["scoring", "scoring,possession"]:
            for lr in [0.0005, 0.0001]:
                for tar in [100, 200]:
                    for env_difficulty in [0.0, 0.2, 0.4]:
                        name = "{0}__{1}__{2}__{3}__{4}__{5}__diff".format(config, map_name, rewards, rnn_dim, lr, tar)
...
# Empty goal, trying to get 1 reward (do this IQL instead)
